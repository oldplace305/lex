...
# ツイート生成プロンプト（AI×収益化切り口）
TWEET_PROMPT = """
あなたはLex Ventures（@{account_name}）のX運用担当。
以下のAIトレンドデータから、「AI×収益化」切り口で日本語3件のツイートを生成してください。

## ルール
- 各ツイート280文字以内（日本語は140文字目安）
- 「英語圏で話題」「日本ではまだ知られていないAIビジネス」「個人でもできる」視点
- ハッシュタグは1-2個（例: #AI活用 #副業 #AIビジネス）
- 煽りすぎない、事実ベースで
- 冒頭にフック（🔥、⚡、📈 など）
- 「このAIで何ができるか」「どう収益化できるか」を必ず含める
...
[
    {{
        "text": "ツイート本文",
        "topic": "元ネタのトピック名",
        "category": "ai/ai_business/ai_tool/side_hustle"
    }}
]
```
""".strip()
